...
// Ambil data tugas listening level 1
$result = $conn->query("SELECT * FROM tugas WHERE tipe = 'listening' AND level = 1 ORDER BY created_at DESC");
$tugas = [];
while ($row = $result->fetch_assoc()) {
  $tugas[] = [
    ...
    'file_path' => 'uploads/listening/' . basename($row['file_path']),
    'gambar_path' => 'uploads/images/' . basename($row['gambar_path']),
...
<head>
  ...
  <title>LittleLingo Listening - Level 1</title>
...
<section class="py-12 px-4 max-w-7xl mx-auto">
  <div class="text-center mb-10">
    <h6 class="text-blue-600 font-semibold uppercase text-sm">GREETINGS AND INTRODUCTION</h6>
  ...
  <div class="text-center mt-12">
    <a href="menu-1.php" class="inline-block bg-blue-500 text-white px-6 py-3 rounded-full font-semibold hover:bg-pink-600 transition">Kembali ke Menu</a>
  </div>
</section>
...
<script>
  ...
  function showTab(index) {
    ...
    // AUDIO
    const audio = document.getElementById("tab-audio");
    audio.querySelector("source").src = tab.file_path;
    audio.load();
